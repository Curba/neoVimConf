...
command! -nargs=0 CompileAndRun call TermWrapper(printf('g++-10 -std=c++20 %s && ./a.out', expand('%')))
command! -nargs=1 -complete=file CompileAndRunWithFile call TermWrapper(printf('g++-10 -std=c++20 %s && ./a.out < %s', expand('%'), <q-args>))
...
augroup CppToolkit
	autocmd!
	if g:os == 'Darwin'
		autocmd FileType cpp nnoremap <leader>fn :!g++-10 -std=c++20 -o %:r % && open -a Terminal './a.out'<CR>
	endif
	autocmd FileType cpp nnoremap <leader>fb :!g++-10 -std=c++20 % && ./a.out<CR>
...
augroup CppToolkit
	autocmd!
	if g:os == 'Darwin'
		autocmd FileType cpp nnoremap <leader>fn :!g++-10 -std=c++20 -o %:r % && open -a Terminal './%:r'<CR>
	endif
	autocmd FileType cpp nnoremap <leader>fb :!g++-10 -std=c++20 -o %:r % && ./%:r<CR>
...
" let g:split_term_resize_cmd = 'resize 7'
